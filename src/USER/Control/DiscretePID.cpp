...
//***************************************************************************
// Author: Renato Caldas (Slight edit by Nikolai Lauvås)                    *
//***************************************************************************

// Local headers.
#include <USER/Control/DiscretePID.hpp>
...
namespace USER
{
  namespace Control
  {
    ...
    void
    DiscretePID::enableParcels(DUNE::Tasks::Task* t, DUNE::IMC::ControlParcel* p)
    {
      m_task = t;
      m_parcel = p;
    ...
    DiscretePID::step(double timestep, float error, float err_derivative)
    {
      ...
      if (m_limit_int)
        m_int_err = DUNE::Math::trimValue(m_int_err, -m_max_int, m_max_int);
      ...
      if (m_limit_out)
        cmd = DUNE::Math::trimValue(cmd, m_lower_limit, m_upper_limit);
